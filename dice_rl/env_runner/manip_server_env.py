...
from utils.filtering import LiveLPFilter
from utils.computer_vision import get_image_transform
from utils.shared_memory.shared_memory_ring_buffer import (
    SharedMemoryRingBuffer,
)
from utils.shared_memory.shared_memory_util import (
    ArraySpec,
)
from utils.usb_util import reset_all_elgato_devices
...
class ManipServerEnv:
    ...
    def schedule_controls(
        ...
        for id in self.id_list:
            ...
            if eoat_cmd is not None and self.server.has_eoat():
    ...
    def get_sparse_observation_from_buffer(self):
        ...
        # read EOAT pos (only if eoat query size is specified, i.e., gripper state is used as obs)
        if self.server.has_eoat() and "eoat" in self.query_sizes["sparse"]:
            for id in self.id_list:
