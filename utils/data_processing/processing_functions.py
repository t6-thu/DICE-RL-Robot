...
from utils.data_processing.episode_data_buffer import (
    VideoData,
    EpisodeDataBuffer,
)
...
from utils import compliance_helpers as ch
from utils.matplotlib_helpers import set_axes_equal
...
def process_one_episode_into_zarr(episode_name, output_root, config):
    ...
    time_offsets = []
    for id in config["id_list"]:
        print(f"[DEBUG] Processing episode {episode_name}, id: {id}")

        if id >= len(data_robot_time_stamps):
            print(f"[ERROR] data_robot_time_stamps has length {len(data_robot_time_stamps)}, but trying to access index {id}")
            # raise IndexError(f"Missing robot timestamp for id={id} in episode {episode_name}")
            return False

        len_data_robot_time_stamps = len(data_robot_time_stamps[id])
        len_rgb_time_stamps = len(data_rgb_time_stamps[id])
        print(f"[DEBUG] len_data_robot_time_stamps: {len_data_robot_time_stamps}, len_rgb_time_stamps: {len_rgb_time_stamps}")
        ratio = len_data_robot_time_stamps / len_rgb_time_stamps
        if ratio < 8 or ratio > 9:
            print(f"[WARNING] The ratio of robot time stamps to RGB time stamps is {ratio}, which is unusual. Please check the data.")
            return False
    
        print(f"[INFO] len(data_robot_time_stamps[{id}]): {len(data_robot_time_stamps[id])}")
