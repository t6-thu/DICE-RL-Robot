...
def raw_to_obs(
    ...
    for id in shape_meta["id_list"]:
        ...
        if "policy_robot0_eef_pos" in shape_meta["obs"].keys():
            ...
            raw_data[f"policy_pose_command_{id}"] = policy_pose7[policy_indices]
            raw_data[f"policy_time_stamps_{id}"] = ts_timestamps
...
def raw_to_action10(
    ...
    """Convert shape_meta.raw data to shape_meta.action for pose9g1 (9D pose + 1D gripper).
    Note: if relative action is used, the relative pose still needs to be computed every time a sample
    is made. This function only converts the whole episode, and does not know what pose to be relative to.
    ...
    for id in id_list:
        ...
        # Convert pose similar to raw_to_action9
        if "policy_robot0_eef_pos" in shape_meta["obs"].keys():
            ts_SE3_command = su.pose7_to_SE3(ts_pose7_command)
            policy_SE3_command = su.pose7_to_SE3(raw_data[f"policy_pose_command_{id}"])
            # compute delta action using relative pose transformation in SE3 space
        ...
        # Align gripper data with robot pose timestamps
        ts_pose_timestamps = raw_data[f"robot_time_stamps_{id}"][:]
        gripper_raw = raw_data[f"gripper_{id}"][:]
        gripper_timestamps = raw_data[f"gripper_time_stamps_{id}"][:]
        
        # Find closest matching timestamps (same approach as raw_to_action15)
        indices = np.searchsorted(gripper_timestamps, ts_pose_timestamps, side="left")
        indices = np.clip(indices, 0, len(gripper_raw) - 1)  # Ensure indices are valid
        gripper_command = gripper_raw[indices]
        
        # Ensure gripper has correct shape
        if len(gripper_command.shape) == 1:
            gripper_command = gripper_command.reshape(-1, 1)
        
        # Combine pose and gripper
        action.append(np.concatenate([ts_pose9_command, gripper_command], axis=-1))

    action_len = min(action_lens)
    action = [x[:action_len] for x in action]

    episode_data["action"] = np.concatenate(action, axis=-1)
    assert episode_data["action"].shape[1] == 10 or episode_data["action"].shape[1] == 20
            ...
            if (ho != hi) or (wo != wi):
                if reshape_mode == "reshape":
                    tf = get_image_transform(
                        input_res=(wi, hi), output_res=(wo, ho), bgr_to_rgb=False, val=val
                    )
...
def sparse_obs_to_obs_sample(
    ...
    if not ignore_rgb:
        obs_rgb_preprocess(obs_sparse, sparse_obs_processed, reshape_mode, shape_meta, val)
...
def obs_to_obs_sample(
    ...
    obs_processed["sparse"], base_pose_mat = sparse_obs_to_obs_sample(
        obs_sparse, shape_meta, reshape_mode, id_list, ignore_rgb, val=val
    )
    if len(obs_dense) > 0:
...
def action10_to_action_sample(
    ...
    """Prepare a sample of actions as labels for the policy network for pose9g1.
    
    This function is used in training. It takes a sample of actions (len = action_horizon)
    and convert the poses in it to be relative to the current pose (id = 0).
    The gripper component is passed through unchanged.
    """
    action_processed = {"sparse": {}, "dense": {}}
    if len(action_sparse) > 0:
        T, D = action_sparse.shape
    ...
    def action10_preprocess(action: np.ndarray, SE3_WBase: np.ndarray):
        if "policy_robot0_eef_pos" in shape_meta["obs"].keys():
            # If using policy commands, action is already relative
            return action
        # Split pose and gripper
        pose9 = action[:, 0:9]
        gripper = action[:, 9:10]
        # Convert pose to relative coordinates
        SE3 = su.pose9_to_SE3(pose9)
        SE3_WBase_inv = su.SE3_inv(SE3_WBase)
        SE3_relative = SE3_WBase_inv @ SE3
        pose9_relative = su.SE3_to_pose9(SE3_relative)
        # Recombine with gripper
        return np.concatenate([pose9_relative, gripper], axis=-1)

    if len(action_sparse) > 0:
        if len(id_list) == 1:
            action_processed["sparse"] = action10_preprocess(action_sparse, base_pose[0])
        else:
            action_processed["sparse"] = np.concatenate(
                [
    ...
    if len(action_dense) > 0:
        ...
        for step in range(H):
            if len(id_list) == 1:
                action_processed["dense"][step] = action10_preprocess(
                    action_dense[step], base_pose[0]
                )
            else:
                action_processed["dense"][step] = np.concatenate(
                    [
                        action10_preprocess(action_dense[step][:, :10], base_pose[0]),
                        action10_preprocess(action_dense[step][:, 10:20], base_pose[1]),
                    ],
                    axis=-1,
                )
