...
from utils.data_plotting import plot_ts_action
...
class DiffusionUnetTimmMod1Policy(BaseImagePolicy):
    def __init__(
        ...
        **kwargs,
        ...
        print(f"[DiffusionPolicy] num_inference_steps = {self.num_inference_steps}")
    ...
    def conditional_sample(
        ...
        **kwargs,
        ...
        # Use provided init_noise or sample random noise
        if init_noise is not None:
            trajectory = init_noise.to(dtype=condition_data.dtype, device=condition_data.device)
        else:
    ...
    def predict_action(
        ...
        init_noise: if provided, use this as initial noise for diffusion sampling
                    shape: (B, action_horizon, action_dim)
        unnormalize_result: if False, return normalized action and encoded features
                           (used by RL finetuning to add residual before unnormalizing)
        """
        ...
        # For RL finetuning: return normalized action + features before unnormalizing
        if not unnormalize_result:
            return {
                "sparse_normalized": sparse_naction_pred,
                "features": sparse_nobs_encode,
            }
    ...
    @torch.no_grad()
    def extract_visual_features(self, obs: Dict) -> torch.Tensor:
        """
        Extract visual features from observations for storing in replay buffer.

        This pre-encodes observations so we don't need to re-encode during training.
        The returned features can be used with predict_action_from_features().

        Args:
            obs: Dict with 'sparse' key containing obs_dict
                 obs['sparse'] has keys from shape_meta['sample']['obs']['sparse']
                 Each key has shape (B, T, ...) - e.g., RGB is (B, T, C, H, W)

        Returns:
            features: Encoded features tensor of shape (B, obs_feature_dim)
        """
        obs_dict_sparse = obs["sparse"]
        nobs_sparse = self.sparse_normalizer.normalize(obs_dict_sparse)
        sparse_nobs_encode = self.obs_encoder(nobs_sparse)
        return sparse_nobs_encode

    def predict_action_from_features(
        self,
        sparse_nobs_encode: torch.Tensor,
        init_noise: torch.Tensor = None,
        unnormalize: bool = True,
    ) -> Dict[str, torch.Tensor]:
        """
        Predict actions using pre-encoded visual features.

        Args:
            sparse_nobs_encode: Pre-encoded features of shape (B, obs_feature_dim)
                                Output from extract_visual_features()
            init_noise: if provided, use this as initial noise for diffusion sampling
                        shape: (B, action_horizon, action_dim)
            unnormalize: if True, unnormalize predicted actions before returning.
                        If False, return normalized actions (for residual RL training).

        Returns:
            result: Dict with 'sparse' key containing predicted actions
                    of shape (B, action_horizon, action_dim)
        """
        batch_size = sparse_nobs_encode.shape[0]
        ...
        if unnormalize:
            ...
            sparse_action_pred = sparse_naction_pred

        # store intermediate results (for dense prediction if needed)
        self.sparse_nobs_encode = sparse_nobs_encode
        self.sparse_naction_pred = sparse_naction_pred

        result = {"sparse": sparse_action_pred, "dense": None}
        return result
