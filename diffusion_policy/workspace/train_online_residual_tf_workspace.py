...
class TrainOnlineResidualTFWorkspace(BaseWorkspace):
    ...
    def run(self, dataset: BaseImageDataset):
        ...
        if dataset.sampler.num_new_episodes > 0 and dataset.sampling_weights is not None:
            ...
            for local_epoch_idx in range(num_epochs):
                ...
                with tqdm.tqdm(
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        nobs_sparse = self.sparse_normalizer.normalize(obs_dict_sparse_without_time)
                        for key in obs_dict_sparse:
                        ...
                        # compute loss
                        raw_loss = self.model.compute_loss(sparse_nobs_encode, time_encode, action_sparse)  # TODO: compute loss
                ...
                if (self.epoch % self.cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        nobs_sparse = dict_apply(nobs_sparse, lambda x: x.to(device))
                        sparse_nobs_encode, time_encode = self.obs_encoder(nobs_sparse)
                        pred_action = self.model.predict_actions(sparse_nobs_encode, time_encode, gt_action.shape)
...
def main(cfg):
    workspace = TrainOnlineResidualTFWorkspace(cfg)
    workspace.run()
