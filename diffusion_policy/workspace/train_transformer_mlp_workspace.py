...
from diffusion_policy.policy.transformer_mlp_policy import (
    TransformerMLPPolicy,
)
...
class TrainTransformerMLPWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: TransformerMLPPolicy = hydra.utils.instantiate(cfg.policy)

        self.ema_model: TransformerMLPPolicy = None
    ...
    def run(self):
        ...
        print("action: ", sample_batch["action"]["dense"].shape)
        print("dataset.action_type: ", dataset.action_type)
        action_dimension = sample_batch["action"]["dense"].shape[-1]
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if (
                            batch_idx == 0
                            or batch["action"]["dense"].shape[0] == batch_size
                        ):
                ...
                def log_action_mse(step_log, category, pred_action, gt_action):
                    pred_naction = dense_normalizer["action"].normalize(pred_action)
                    gt_naction = dense_normalizer["action"].normalize(gt_action)
                    B, T, _, _ = pred_naction.shape
                    pred_naction_reshape = pred_naction.view(B, T, -1, action_dimension)
                    gt_naction_reshape = gt_naction.view(B, T, -1, action_dimension)
                    step_log[f"{category}_dense_naction_mse_error"] = (
                        torch.nn.functional.mse_loss(
                            pred_naction_reshape, gt_naction_reshape
                        )
                    )
                    # step_log[f'{category}_dense_naction_mse_error_pos'] = torch.nn.functional.mse_loss(pred_naction_dense[..., :3], gt_naction_dense[..., :3])
                    # step_log[f'{category}_dense_naction_mse_error_rot'] = torch.nn.functional.mse_loss(pred_naction_dense[..., 3:9], gt_naction_dense[..., 3:9])
                ...
                if (
                    ...
                    with torch.no_grad():
                        ...
                        gt_action = batch["action"]["dense"]
                        pred_action = policy.predict_action(
                            batch["obs"], unnormalize_result=True
                        )
...
def main(cfg):
    workspace = TrainTransformerMLPWorkspace(cfg)
    workspace.run()
