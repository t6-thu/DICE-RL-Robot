...
from utils.common_type_conversions import (
    ...
    action10_to_action_sample,
    action19_to_action_sample,
    action21_to_action_sample,
)

from utils.trajectory import LinearInterpolator
from utils.spatial_math import rot6_to_SO3, SO3_to_rot6d
from utils.trajectory import LinearTransformationInterpolator
from utils.data_processing.indexing import get_dense_query_points_in_horizon
...
class VirtualTargetDataset(BaseImageDataset):
    ...
    def __init__(
        ...
        include_next_obs: bool = False,           # <— new argument
    ):
        ...
        if (
            ...
            shape_meta["action"]["shape"][0] == 10
            or shape_meta["action"]["shape"][0] == 20
        ):
            action_type = "pose9g1"
        elif (
        ...
        if action_type == "pose3":
            ...
            action_to_action_sample = action10_to_action_sample
        ...
        self.flag_has_dense = "dense" in shape_meta["sample"]["obs"] # false
        self.normalize_wrench = normalize_wrench

        print('len of dataset:', len(self))
    ...
    def get_normalizer(self, **kwargs) -> tuple:
        ...
        for i in range(len(self.id_list)):
            ...
            if AL == 10:
                # Add gripper normalization for pose9g1
                sparse_action_normalizers.append(
                    get_range_normalizer_from_stat(
                        array_to_stats(
                            data_cache_sparse["action"][..., i * AL + 9 : i * AL + 10]
                        )
                    )
                )  # gripper
            if AL < 19:
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.sampler.include_next_obs:
            obs_dict, action_array, next_obs_dict = self.sampler.sample_sequence(idx)
            torch_data = {
                "obs": dict_apply(obs_dict, torch.from_numpy),
                "action": dict_apply(action_array, torch.from_numpy),
                "obs_next": dict_apply(next_obs_dict, torch.from_numpy),
            }
        else:
...
#     # Optional: pretty print config for visual confirmation
#     def print_config(config):
#         print(OmegaConf.to_yaml(OmegaConf.create(config)))

#     # Manually specify config as in the YAML file
#     config = {
#         "shape_meta": {
#             "id_list": [0],
#             "raw": {
...
#                 "obs": {
#                     "sparse": {
#                         "rgb_0": {"horizon": 1, "down_sample_steps": 10},
#                         "robot0_eef_pos": {"horizon": 2, "down_sample_steps": 5},
#                         "robot0_eef_rot_axis_angle": {"horizon": 2, "down_sample_steps": 5},
#                     },
#                 },
#                 "action": {
#                     "sparse": {
#                         "horizon": 32,
#                         "down_sample_steps": 50,
#                     }
#                 },
#                 "training_duration_per_sparse_query": 3300,
#             },
#         },
#         "dataset_path": "/store/real/zhanyis/data/real_processed/belt_zhanyi_test_processed",
#         "sparse_query_frequency_down_sample_steps": 88,
#         "action_padding": True,
...
#     print("[main] Debug config:")
#     print_config(config)

#     # Create dataset instance
#     dataset = VirtualTargetDataset(
...
#     print("[main] Dataset length:", len(dataset))
#     from torchvision.utils import save_image
#     # Try fetching one sample
#     sample = dataset[0]
#     print("[main] Sample keys:", sample.keys())
#     print("[main] Obs keys:", sample["obs"].keys())
#     print("[main] Action shape:", sample["action"]["sparse"].shape)
...
#     for (idx, batch) in enumerate(tqdm(dataloader, desc="Iterating dataset")):
#         print(f"[main] Batch {idx}")
#         print("[main] Batch current obs keys:", batch["obs"]["sparse"].keys())
#         print("[main] Batch action shape:", batch["action"]["sparse"].shape)
#         print("[main] Batch next obs keys:", batch["next_obs"]["sparse"].keys() if "next_obs" in batch else "No next_obs")
#         for key, value in batch["obs"]["sparse"].items():
#             print(f"[main] current obs {key} shape: {value.shape}")
#             # if'rgb' not in key:
#             #     print(value[0][-1])
#             # else:
#             if 'rgb' in key:
#                 print('range of current rgb:', value[0].min(), value[0].max())
#         print("[main] Batch next obs keys:", batch["next_obs"]["sparse"].keys())
#         for key, value in batch["next_obs"]["sparse"].items():
#             print(f"[main] next obs {key} shape: {value.shape}")
#             # if'rgb' not in key:
#             #     print(value[0][-1])
#             # else:
#             if 'rgb' in key:
#                 print('range of next rgb:', value[0].min(), value[0].max())

#         curr = batch["obs"]["sparse"]["rgb_0"][0, 0]   
#         nxt  = batch["next_obs"]["sparse"]["rgb_0"][0, 0]

#         # side-by-side: cat along width (dim=2)
#         combined = torch.cat([curr, nxt], dim=2)  # shape [3, 224, 224*2]

#         # save as PNG
#         save_image(combined, f"/store/real/zhanyis/pyrite/debug_images/{idx}.png")
